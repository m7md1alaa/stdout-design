...
describe("applyUpdate", () => {
  test("writes studio.config.ts with merged config", async () => {
    const dir = inTempDir();
    try {
      const existing: StudioConfig = { scaffoldVersion: "0.1.0", presets: [], templates: {} };
  ...
  test("updates @stdout-design/cli dep in package.json", async () => {
    ...
    try {
      ...
      writeFileSync(path.resolve(dir, "package.json"), JSON.stringify(pkg, null, 2));
      ...
      const updated = JSON.parse(readFileSync(path.resolve(dir, "package.json"), "utf-8"));
  ...
  test("leaves package.json unchanged when dep is already current", async () => {
    const dir = inTempDir();
    try {
      const pkg = { dependencies: { "@stdout-design/cli": "^0.2.0" }, name: "test" };
      writeFileSync(path.resolve(dir, "package.json"), JSON.stringify(pkg, null, 2));

      const existing: StudioConfig = { scaffoldVersion: "0.2.0", presets: [], templates: {} };
      const plan = planUpdate(existing, defaults, "0.2.0", new Set());

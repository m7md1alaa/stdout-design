...
| --- | --- | --- |
| `templateId` | Yes | A template ID from `studio.config.ts` |
| `props` | Yes | Raw prop values (validated server-side against the template's schema) |
| `preset` | No | Preset ID defaults to `config.defaultPreset` |
| `locale` | No | Locale code merges that locale's translation data into props |
| `autoDetected` | No | Set `true` to skip locale-file lookup (used by the studio's RTL auto-detect) |

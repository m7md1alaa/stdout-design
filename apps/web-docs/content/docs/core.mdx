...
`@stdout-design/core` builds on [Takumi](https://takumi.kane.tw/docs) for the actual render engine (`renderToPixels`, `measureTemplate`, and the `RenderOptions` you pass to them are Takumi's). For anything below this page fonts, stylesheets, image/emoji handling at the engine level, or an option this page doesn't cover check [Takumi's reference](https://takumi.kane.tw/docs/reference).
...
The third argument to `renderToPixels`/`measureTemplate` is Takumi's own `RenderOptions` (`fonts`, `fontFamilies`, `lang`, `images`, `stylesheets`, …) see [Takumi's reference](https://takumi.kane.tw/docs/reference) for the full option list.
...
<Cards>
  ...
  <Card title="Takumi Reference" href="https://takumi.kane.tw/docs/reference" />
</Cards>
